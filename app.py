...
def hesapla_rpt(df, aylar_sim, mevsimsellik, h_gun, m_moq, v_kat, lead):
    """
    DÜZELTME (çakışan sipariş / stok şişmesi):
    Eskiden RPT, AY BAŞI pozisyonuna göre hesaplanıyordu (devreden + gelen mal),
    satış hiç görülmeden. Bu durumda örneğin cover 180 iken satış düşürüp ay
    sonunda 130'a inecek olsa bile, eğer o ay zaten yolda bir sipariş (gelen mal)
    ...
    for i, ay in enumerate(aylar_sim):
        ...
        if i < lead:
            ...
            rpt = np.where(
                ...
                np.where(
                    ihtiyac_var,
                    np.where(ihtiyac <= m_moq, m_moq, np.ceil(ihtiyac / np.where(v_kat > 0, v_kat, 1)) * v_kat),
                    0
                )
            )
        ...
        df[f'{ay}_RPT'] = rpt
        df[f'{ay}_Kapanis_Stogu'] = kapanis_stogu
...
# --- SONUÇLAR ---
# Çakışan sipariş nedeniyle ertelenen ihtiyaç notları
if 'RPT_Notlari' in df.columns:
