...
st.set_page_config(
    page_title="RPT HESAPLAMA PROGRAMI",
    layout="wide",
...
def veriyi_yukle(dosya_bytes):
    ...
    gerekli = ["SKU", "Acilis_Stogu", "Son_3_Ay_Ort_Satis", "Ana Kategori", "Ürün Grubu", "Ürün Adı"]
...
def hesapla_rpt(df, aylar_sim, mevsimsellik, h_gun, m_moq, v_kat, lead, tavan_cover):
    ...
    for i, ay in enumerate(aylar_sim):
        ...
        gelen = df[ay].fillna(0).to_numpy(float) if ay in df.columns else np.zeros(len(df))
...
with st.sidebar:
    ...
    with st.form("parametre_formu"):
        ...
        f_tavan = st.number_input("Cover Tavanı (gün)", value=st.session_state["p_tavan"], step=10, help="Hiçbir SKU'ya bu günün üstüne RPT yazılmaz")
    ...
    if grup_listesi:
        ...
        if st.session_state["gecici_kurallar"]:
            ...
            h1.markdown("**ÜRÜN GRUBU**")
            h2.markdown("**MIN COVER**")
            h3.markdown("**MOQ (AD)**")
            ...
            for idx, k in enumerate(st.session_state["gecici_kurallar"]):
                ...
                if c0.button("🗑", key=f"del_{idx}", help="Sil"):
                    ...
                    st.rerun()
                c1.markdown(f"{k['Ürün Grubu']}")
                c2.markdown(f"{k['Cover']}")
                c3.markdown(f"{k['MOQ']}")
...
ozet_df = pd.DataFrame(
    ...
    [{"Ürün Grubu": k["Ürün Grubu"], "Min Cover (gün)": k["Cover"], "MOQ (adet)": k["MOQ"]} for k in st.session_state["aktif_kurallar"]]
)
...
if df is None or not hasattr(df, "columns"):
    st.error("Hesaplama sonrası df oluşmadı. Lütfen hesapla_rpt() dönüşünü kontrol edin.")
    st.stop()
...
if cover_cols:
    ...
    if tavan_asan_adet > 0:
        st.warning(
            f"⚠️ {tavan_asan_adet} SKU'nun mevcut açılış stoğu zaten {v_tavan} gün tavanının üzerinde. "
