...
def check_password():
    ...
    if ip in auth_state["lockouts"] and time.time() < auth_state["lockouts"][ip]:
        st.error("🚨 Çok sayıda hatalı giriş! 30 dakika bloke edildiniz.")
        return False
    ...
    def password_entered():
        if st.session_state["password"] == st.secrets["APP_PASSWORD"]: 
            ...
            if auth_state["attempts"][ip] >= 4: auth_state["lockouts"][ip] = time.time() + 1800
...
# --- UYGULAMA ---
st.title(" RPT ve Cover Hesaplama Programı")
...
with st.sidebar:
    ...
    mevsimsellik = dict(zip(st.data_editor(pd.DataFrame({"Ay": aylar_sim, "Katsayi": katsayilar}), hide_index=True)["Ay"], st.data_editor(pd.DataFrame({"Ay": aylar_sim, "Katsayi": katsayilar}), hide_index=True)["Katsayi"]))
...
if file:
    ...
    satis_cols = [f"{ay}_Beklenen_Satis" for ay in aylar_sim]
    stok_cols = [f"{ay}_Kapanis_Stogu" for ay in aylar_sim]
    cover_cols = [f"{ay}_Cover_Gun" for ay in aylar_sim]
    rpt_cols = [f"{ay}_RPT" for ay in aylar_sim]
    cols = base + satis_cols + stok_cols + cover_cols + rpt_cols
